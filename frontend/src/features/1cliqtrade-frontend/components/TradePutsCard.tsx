/**
 * TradePutsCard - Phase 4 Component
 * Trading buttons for BUY/SELL PUT options with LTP and LIMIT pricing
 */
...
const logger = createLogger('TradePutsCard');

interface TradePutsCardProps {
    quantity: number;
    buyLevel: number;
    sellLevel: number;
...
export function TradePutsCard({
    ...
    // Subscribe to put option price (e.g., NIFTY26APR2420800PE)
    const putSymbol = useMemo(() => {
        // This would be set by user selecting a strike price
        // For now, placeholder
        return undefined;
    }, []);

    useWebSocketLivePrice(putSymbol);
    ...
    return (
        <div className={`p-1.5 bg-white border border-slate-200 rounded shadow-sm ${className}`}>
            ...
            <div className="mb-1 pb-0.5 border-b border-slate-200">
                <h3 className="text-xs font-bold text-slate-900">🔵 PUT</h3>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full mx-4">
                        ...
                        <input
                            ...
                            placeholder="0.00"
                            step="0.01"
                            className="w-full px-3 py-2 border-2 border-slate-300 rounded mb-4 focus:outline-none focus:border-blue-500"
                            autoFocus
                        />
